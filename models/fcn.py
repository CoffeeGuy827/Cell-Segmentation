...
class FCN8s(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.relu(self.deconv1(x5))               # size=(N, 512, x.H/16, x.W/16)
        if score.shape[2:] != x4.shape[2:] :
            score = F.interpolate(score, x4.shape[2:])
        score = self.bn1(score + x4)                      # element-wise add, size=(N, 512, x.H/16, x.W/16)
        score = self.relu(self.deconv2(score))            # size=(N, 256, x.H/8, x.W/8)
        if score.shape[2:] != x3.shape[2:] :
            score = F.interpolate(score, x3.shape[2:])
        ...
        if score.shape[2:] != x.shape[2:] :
            score = F.interpolate(score, x.shape[2:])
        return score  # size=(N, n_class, x.H/1, x.W/1)
...
def fcn8s(num_classes) :
    ...
    return FCN8s(pretrained_net=vgg_model, n_class=num_classes)

def fcn(num_classes) :
    vgg_model = VGGNet(requires_grad=True)
    return FCNs(pretrained_net=vgg_model, n_class=num_classes)

if __name__ == "__main__":
    batch_size, n_class, h, w = 1, 1, 160, 160
    vgg_model = VGGNet(requires_grad=True)
